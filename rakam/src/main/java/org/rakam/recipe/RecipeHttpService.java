...
public class RecipeHttpService extends HttpService {
    ...
    @POST
    @Path("/export")
    public void export(RakamHttpRequest request) throws JsonProcessingException {
        request.bodyHandler(s -> {
            ExportRequest data = JsonHelper.read(s, ExportRequest.class);
            Recipe export = installer.export(data.project);

            ByteBuf buffer;
            try {
                buffer = Unpooled.wrappedBuffer((data.type != null ? data.type.mapper : ExportType.YAML.mapper)
            ...
            response.headers().add(CONTENT_TYPE, (data.type != null ? data.type.contentType : ExportType.YAML.contentType));
    ...
    public static class ExportRequest {
        ...
        @JsonCreator
        public ExportRequest(@ApiParam(name="project") String project,
                             @ApiParam(name="type", required = false) ExportType type) {
