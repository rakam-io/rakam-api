 ...
 */
public class StreamQueryExecutionPlanVisitor
        extends PlanVisitor<Void, Processor>
{
    ...
    public StreamQueryExecutionPlanVisitor(Metadata metadata, Map<Symbol, Type> types)
    {
    ...
    private Processor planGlobalAggregation(AggregationNode node, Void context)
    {
        int outputChannel = 0;
    ...
    public Processor visitAggregation(AggregationNode node, Void context)
    {
        ...
        if (node.getGroupBy().isEmpty()) {
            return planGlobalAggregation(node, context);
        }
