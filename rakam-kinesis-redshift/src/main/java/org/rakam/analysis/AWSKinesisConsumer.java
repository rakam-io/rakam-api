...
import redshift.KinesisMessageModelRedshiftTransformer;
...
public class AWSKinesisConsumer implements IRecordProcessor {
    private final KinesisMessageModelRedshiftTransformer transformer;
    ...
    public AWSKinesisConsumer(KinesisConnectorConfiguration config, ContinuousQueryService continuousQueryService, Metastore metastore) {
        this.buffer = new BasicMemoryBuffer<>(config);
        this.emitter = new S3ManifestEmitter(config);
        this.transformer = new KinesisMessageModelRedshiftTransformer(metastore, config);
