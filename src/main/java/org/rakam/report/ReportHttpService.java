...
public class ReportHttpService implements HttpService {
    ...
    @Inject
    public ReportHttpService(ReportMetadataStore database, EventSchemaMetastore metastore, PrestoConfig prestoConfig) {
    ...
    public void execute(RakamHttpRequest request) {
        if (Objects.equals(request.headers().get(HttpHeaders.Names.ACCEPT), "text/event-stream")) {
            ...
            new PrestoQueryExecutor(response, prestoConfig.getAddress()).executeQuery(builder.toString());
        }
    }
