...
//        try {
            ...
            if (isColumnar) {
                ObjectNode columns = JsonHelper.jsonObject();
                for (int i = 1; i <= metaData.getColumnCount(); i++) {
                    columns.putArray(metaData.getColumnName(i));
                }

                while (rs.next()) {
                    extractColumnsFromResultSet(rs, metaData, columns);
                }

                jsonNodes.set("result", columns);
                if (metaData.getColumnCount() > 0) {
                    jsonNodes.put("total", columns.get(metaData.getColumnName(1)).size());
                }
            } else {
