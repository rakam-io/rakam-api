...
public class EventAnalyzer {
    public JsonObject analyze(JsonObject jsonObject1) {
        return null;
    }
//
...
//        FilterScript filter = AnalysisRuleParser.getFilter(query.getJsonObject("filter"));
//
//        switch (analysis) {
//            case ANALYSIS_TIMESERIES:
//                final Interval interval;
//                try {
//                    interval = AnalysisRuleParser.getInterval(query.getValue("interval"));
//                } catch (IllegalArgumentException e) {
...
//        List<Integer> keys;
//        try {
//            keys = getTimeFrame(query.getValue("frame"), interval, query.getString("start"), query.getString("end"));
//        } catch (IllegalArgumentException e) {
//            return returnError(e.getMessage());
//        }
//
...
//                .put("start_date", Instant.ofEpochSecond(keys.get(0)).atZone(timezone).format(formatter))
//                .put("end_date", Instant.ofEpochSecond(keys.get(keys.size() - 1)).atZone(timezone).format(formatter));
...
//                        long sum = 0;
//                        for (Map.Entry<String, Object> field : result)
//                            sum += ((Number) field.getValue()).longValue();
//
//                        calculatedResult.put(Instant.ofEpochSecond(key).atZone(timezone).format(formatter), sum);
//                    }
//                    break;
//                case MINIMUM_X:
//                case MAXIMUM_X:
...
//                        JsonObject result = fetchNonGroupingTimeSeries(mRule, objects, aggAnalysis, items.intValue());
//                        Long val = null;
//                        for (String field : result.getValueNames()) {
...
//                        }
//
//                        calculatedResult.put(Instant.ofEpochSecond(key).atZone(timezone).format(formatter), val);
//                    }
//
//                    break;
//                case AVERAGE_X:
...
//                            span.next();
//                        }
//                        calculatedResult.put(formatTime(key), count > 0 ? sum / count : 0);
//                    }
//                    break;
//                case COUNT_UNIQUE_X:
//                    for (Integer key : keys) {
...
//                                hll.union(dbHll);
//                        }
//                        calculatedResult.put(formatTime(key), hll.cardinality());
//                    }
//                    break;
...
//                            span.next();
//                        }
//                        calculatedResult.put(formatTime(key), new JsonArray(set.stream().toArray(String[]::new)));
//                    }
//                    break;
//            }
//        else {
...
//                        for (String timestamp : result.getValueNames()) {
//                            JsonObject timestampItems = result.getJsonObject(timestamp);
//                            for (String item : timestampItems.getValueNames()) {
//                                Long existingCounter = (Long) sum.getOrDefault(item, 0L);
//                                sum.put(item, existingCounter + timestampItems.getLong(item));
//                            }
//                        }
//                        calculatedResult.put(Instant.ofEpochSecond(key).atZone(timezone).format(formatter), new JsonObject(sum));
//                    }
//                    break;
//                case MINIMUM_X:
//                case MAXIMUM_X:
...
//                        calculatedResult.put(Instant.ofEpochSecond(key).atZone(timezone).format(formatter), new JsonObject(collect));
//
//                    }
//
//                    break;
...
//                        hll.forEach((k, v) -> grouped.put(k, v.cardinality()));
//                        calculatedResult.put(Instant.ofEpochSecond(key).atZone(timezone).format(formatter), grouped);
//                    }
//                    break;
//                case SELECT_UNIQUE_X:
//                    for (Integer key : keys) {
...
//                                } else {
//                                    map.put(entry.getKey(), new JsonArray(entry.getValue().toArray()));
//                                }
//                            }
//                            span.next();
//                        }
//                        calculatedResult.put(Instant.ofEpochSecond(key).atZone(timezone).format(formatter), map);
//                    }
//                    break;
//
//            }
//        }
//
//        json.put("result", calculatedResult);
//        if (!mRule.batch_status && mRule.strategy != AnalysisRuleStrategy.REAL_TIME)
//            json.put("info", "batch results are not combined yet");
//
//        return json;
...
//        Object result;
//        Long items = ConversionUtil.toLong(query.getValue("items"), 10L);
//        if (items == null)
//            return returnError("items parameter is not numeric");
//
//        JsonObject json = new JsonObject();
...
//                List<Integer> keys;
//                try {
//                    keys = getTimeFrame(query.getValue("frame"), ((TimeSeriesAggregationRule) rule).interval, query.getValue("start"), query.getValue("end"));
//                } catch (IllegalArgumentException e) {
//                    return returnError(e.getMessage());
//                }
//
...
//                        json.put(item.getKey(), item.getValue().getCount());
//                    }
//                    break;
//                }
//            case SUM_X:
//                if (rule.type == AVERAGE_X) {
//                    for (Map.Entry<String, AverageCounter> item : metricStorageAdapter.getMetricGroupByAverageCounters(rule_id, items).entrySet()) {
//                        json.put(item.getKey(), item.getValue().getSum());
//                    }
//                    break;
//                }
//            case COUNT:
...
//                    json.put(counter.getKey(), counter.getValue());
//                }
//                break;
//            case AVERAGE_X:
//                for (Map.Entry<String, AverageCounter> item : metricStorageAdapter.getMetricGroupByAverageCounters(rule_id, items).entrySet()) {
//                    json.put(item.getKey(), item.getValue().getValue());
//                }
//                break;
//            case COUNT_UNIQUE_X:
...
//                    json.put(item.getKey(), new JsonArray(item.getValue().toArray()));
//                }
//                break;
//        }
//        return json;
...
//                    results.put(formatTime(time), adapter.getTimeSeriesCounter(rule_id, time));
//                }
//                break;
//            case SUM_X:
...
//                        results.put(formatTime(time), counter == null ? 0 : counter.getSum());
//                    }
//                } else {
//                    for (Integer time : keys) {
//                        TimeSeriesStreamAdapter adapter = now == time ? memoryTimeSeriesStorageAdapter : diskTimeSeriesStorageAdapter;
//                        results.put(formatTime(time), Objects.firstNonNull(adapter.getTimeSeriesCounter(rule_id, time), 0));
//                    }
//                }
//                break;
//            case COUNT_X:
...
//                        TimeSeriesStreamAdapter adapter = now == time ? memoryTimeSeriesStorageAdapter : diskTimeSeriesStorageAdapter;
//                        AverageCounter counter = adapter.getTimeSeriesAverageCounter(rule_id, time);
//                        results.put(formatTime(time), counter == null ? 0 : counter.getCount());
//                    }
//                } else {
//                    for (Integer time : keys) {
//                        TimeSeriesStreamAdapter adapter = now == time ? memoryTimeSeriesStorageAdapter : diskTimeSeriesStorageAdapter;
...
//                    results.put(formatTime(time), set == null ? new JsonArray() : new JsonArray(set));
//                }
//                break;
//            case COUNT_UNIQUE_X:
//                for (Integer time : keys) {
//                    TimeSeriesStreamAdapter adapter = now == time ? memoryTimeSeriesStorageAdapter : diskTimeSeriesStorageAdapter;
//                    results.put(formatTime(time), Objects.firstNonNull(adapter.getTimeSeriesStringCount(rule_id, time), 0));
//                }
//                break;
//            case AVERAGE_X:
//                for (Integer time : keys) {
...
//                    results.put(formatTime(time), averageCounter != null ? averageCounter.getValue() : 0);
//                }
//                break;
//            case COUNT:
//                for (Integer time : keys) {
...
//                            obj.put(item.getKey(), new JsonArray(item.getValue().toArray()));
//                        }
//                    }
//                    results.put(Instant.ofEpochSecond(time).atZone(timezone).format(formatter), obj);
//                }
//                break;
//            case COUNT_UNIQUE_X:
//                for (Integer time : keys) {
...
//                            arr.put(item.getKey(), item.getValue());
//                        }
//
//                    results.put(Instant.ofEpochSecond(time).atZone(timezone).format(formatter), arr);
//                }
//                break;
//            case AVERAGE_X:
//                for (Integer time : keys) {
...
//                            arr.put(item.getKey(), item.getValue().getValue());
//                        }
//                    }
//
//                    results.put(Instant.ofEpochSecond(time).atZone(timezone).format(formatter), arr);
...
//                                arr.put(item.getKey(), aggAnalysis == SUM_X ? value.getSum() : value.getCount());
//                            }
//                        }
//
//                        results.put(Instant.ofEpochSecond(time).atZone(timezone).format(formatter), arr);
